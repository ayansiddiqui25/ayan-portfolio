...
export default async function ProjectPage({ params }: Props) {
  ...
  return <main className="case-study page-width">
    ...
    {story.considerations && <section className="case-notes"><h2>Engineering considerations</h2><ul className="detail-list">{story.considerations.map(point => <li key={point}>{point}</li>)}</ul></section>}
    {story.considerations && !project.gallery?.length && <section className="case-media" aria-labelledby="project-images"><h2 id="project-images">Design &amp; drawings</h2><div className="image-placeholder" role="img" aria-label="Project images to come"><span className="placeholder-corners" aria-hidden="true" /><span className="placeholder-caption">CAD images to come</span></div></section>}
